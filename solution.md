...
- 修改方式：
  ...
  - 如果需要保留“当前赛季类型”的视觉层级，可改成更中性的 `Real Season` / `Fixture mode`，但你这条需求更像是彻底移除。
...
- 修改方式：
  ...
  - 在弹窗里把“排位赛 / 冲刺赛 / 正赛 / 进站”等块改成可点击或可展开项，点击后显示该阶段具体分数来源与细则。
  - 没有冲刺赛的周末不显示“冲刺赛”框。
- 验收点：点“排位赛”能看到具体怎么拿分，没冲刺的周末不会硬塞一个空框。
...
## Assumptions
- 存档使用浏览器本地存储，不做云端同步。
- 车队配色以赛季和车队名归一化为准，不额外引入外部数据源。
- 你提到的 `T3` 已按“右侧市场区域未撑满高度”理解，不按“列表条目数量不足”处理。
- `T5` 的“第一场前不显示”理解为首轮处理前不展示任何价格变化。
- 未特别说明的赛季配色回退，一律统一到 2022 版。
